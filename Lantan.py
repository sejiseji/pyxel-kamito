...
class Lantan:
    ...
    def draw(self, min_x=0, max_x=300):
        ###x_offsetを加味して画面内(min_x〜max_x)にある場合のみ描画処理を行う
        if self.draw_x > min_x -16 and self.draw_x < max_x + 16:
            ###灯籠の描画
            pyxel.blt(self.draw_x, self.y, 1, 128, 0 + 21*(((pyxel.frame_count + self.start_frame)//10)%8), 16, 21, 0)
