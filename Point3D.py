    ...
    def update(self, center, radius):
        ...
        if self.cache_ON:
            ...
            if len(self.coordinates_cache) <= num_points:
                self.coordinates_cache.append((self.x, self.y, self.z))
        ...
        for trail_x, trail_y, _ in reversed(self.trail):  # 逆順で軌跡をループ
            ...
            if self.pallete == 0:
                ...
                if index == 4:
                    ...
                    color = 4
            pyxel.line(prev_x -scroll_x, prev_y, trail_x -scroll_x, trail_y, color)  # 軌跡のラインを描画
            prev_x, prev_y = trail_x, trail_y
            index -= 1
        if self.pallete == 0:        
            pyxel.pset(self.x -scroll_x, self.y, 7)  # 主点を描画
        elif self.pallete == 1:
            pyxel.pset(self.x -scroll_x, self.y, 10)  # 主点を描画
        elif self.pallete == 2:
            pyxel.pset(self.x -scroll_x, self.y, 3) 
